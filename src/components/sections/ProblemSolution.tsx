      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <ul className="space-y-4">
              {painPoints.map((point, i) => (
                <li key={i} className="flex items-start gap-3 text-gray-400 text-sm sm:text-base">
                  <point.icon className="w-5 h-5 text-red-500/50 shrink-0 mt-0.5" />
                  {point.text}
                </li>
              ))}
            </ul>
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="p-8 rounded-3xl bg-accent/5 border border-accent/20 shadow-[0_0_40px_rgba(0,230,192,0.05)]"
          >
            ...
            <ul className="space-y-4">
              {solutions.map((sol, i) => (
                <li key={i} className="flex items-start gap-3 text-gray-200 text-sm sm:text-base font-medium">
                  <sol.icon className="w-5 h-5 text-accent shrink-0 mt-0.5" />
                  {sol.text}
                </li>
              ))}
            </ul>
